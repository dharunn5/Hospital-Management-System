    ...
    React.useEffect(() => {
        const fetchProfile = async () => {
            ...
            try {
                ...
                if (!img) img = assets.profile_pic;
                else if (img.startsWith('/uploads')) img = `http://localhost:5000${img}`;
                setProfilePic(img);
            } catch { setProfilePic(assets.profile_pic); }
        };
        fetchProfile();
                ...
                <div className='flex items-center gap-4'>
                    {
                        token
                            ? <div className='flex items-center gap-2 cursor-pointer group relative'>
                                <img className='w-8 rounded-full' src={profilePic} alt="userpicture" />
